...
async def landing_page(request: Request):
    return templates.TemplateResponse("landing.html", {"request": request, "title": "LOVACRM - Landing"})
...
async def dashboard(request: Request, session: Session = Depends(get_session)):
    ...
    has_data = len(contacts) > 0 and any(
        client_stats[c.id]["visits_confirmed"] > 0 or client_stats[c.id]["avg_spend"] 
        for c in contacts
    )
    ...
    # Средний чек (prosečan račun)
    all_spends = [b.spend_eur for b in all_bookings if b.spend_eur is not None]
    avg_spend = sum(all_spends) / len(all_spends) if all_spends else 0
    
    # Количество гостей (broj gostiju) - уникальные контакты с резервациями
    guests_with_bookings = len(set(b.contact_id for b in all_bookings))
    
    # Рейтинг Google Maps (пока статичный, можно добавить в модель)
    google_rating = 4.6
    
    # Посещаемость (dolaznost) - процент подтвержденных резерваций
    confirmed_bookings = len([b for b in all_bookings if b.status in (BookingStatus.POTVRDJENO, "potvrđeno", "potvrdjeno")])
    attendance_rate = (confirmed_bookings / total_bookings * 100) if total_bookings > 0 else 0
    
    # Новые vs постоянные клиенты
    new_clients = len([c for c in contacts if client_stats[c.id]["visits_confirmed"] <= 1])
    regular_clients = len([c for c in contacts if client_stats[c.id]["visits_confirmed"] > 1])
    new_clients_percent = (new_clients / total_contacts * 100) if total_contacts > 0 else 0
    regular_clients_percent = (regular_clients / total_contacts * 100) if total_contacts > 0 else 0
    
    # Retention (среднее количество посещений на клиента)
    retention = sum(client_stats[c.id]["visits_confirmed"] for c in contacts) / total_contacts if total_contacts > 0 else 0
    
    # Маркетинговые источники (пока статичные, можно добавить в модель)
    marketing_data = {
        "instagram": 21,
        "promoter": 25,
        "random": 24,
    ...
    # Потребление (potrošnja) - общая сумма трат
    total_spend = sum(all_spends) if all_spends else 0
    
    # Заполненность по месяцам (пока статичная, можно вычислить из дат резерваций)
    monthly_occupancy = {
        "feb": 50,
        "mar": 76,
        "apr": 90,
    ...
    response = templates.TemplateResponse("dashboard.html", {
        ...
        "new_clients_percent": new_clients_percent,
        "regular_clients_percent": regular_clients_percent,
        "retention": retention,
