...
def send_upcoming_expiry_notification_client(email: str, business_name: str, expiry_date: datetime) -> bool:
    ...
    try:
        ...
        subject = "⚠️ Vaš probni period se završava uskoro - LovaCRM"
        message = f"""
Zdravo, {business_name}!

Vaš probni period na LovaCRM sistemu se završava uskoro.

📅 Datum isteka: {expiry_date.strftime('%d.%m.%Y %H:%M')}
⏰ Preostalo vreme: manje od 3 sata
...
def send_upcoming_expiry_notification_admin(business_name: str, business_email: str, expiry_date: datetime) -> bool:
    ...
    try:
        ...
        subject = f"⚠️ Upozorenje: {business_name} - probni period se završava uskoro"
        message = f"""
Zdravo, Admin!

Biznis {business_name} ima probni period koji se završava uskoro.
...
def check_upcoming_expirations(session: Session) -> int:
    """Проверяет бизнесы, у которых скоро истечет срок (за 3 часа) и отправляет уведомления"""
    now = datetime.utcnow()
    three_hours_later = now + timedelta(hours=3)
    
    # Находим бизнесы, у которых срок истекает в ближайшие 3 часа
    upcoming_expirations = session.exec(
        select(Business).where(
            Business.status == BusinessStatus.ACTIVE,
            Business.access_end_date > now,
            Business.access_end_date <= three_hours_later
        )
    ).all()
